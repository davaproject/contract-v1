...
contract AvatarV1_1 is AvatarBase {
    ...
    function externalImgUri() public view override returns (string memory) {
        ...
        for (uint256 i = 0; i < frames.length; i += 1) {
            queries[i] = URICompiler.Query(
                string(abi.encodePacked("z", frames[i].zIndex.toString())),
                string(
                    abi.encodePacked(
            ...
            if (parts[i].collection != address(0x0)) {
                queries[layerAmount] = URICompiler.Query(
                    string(
                        abi.encodePacked(
                            "z",
                            IPartCollection(parts[i].collection)
                                .zIndex(parts[i].id)
                                .toString()
                        )
                    ),
                    string(
                        abi.encodePacked(
                            uint256(uint160(parts[i].collection)).toHexString(
                                20
                            ),
                            "[",
